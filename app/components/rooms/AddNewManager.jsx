...
export default function AddNewManager({ setAddManager }) {
  ...
  const handleAddManager = async (e)=>{
    ...
    try {
        ...
        toast.success('הוספת מנהל בהצלחה')
        setAddManager(false)
    } catch (error) {
        toast.error(error.response?.data?.message||'שגיאה בעת הוספת מנהל')
        setLoading(false)
    }
  }
      ...
      <div className="bg-blue-600 px-6 py-4">
        <div className="flex items-center justify-between gap-3 w-full">
          <div className="flex items-center justify-center gap-3 mx-auto">
            ...
            <h2 className="text-2xl font-bold text-white">הוספת מנהל חדש</h2>
          </div>
          <div
