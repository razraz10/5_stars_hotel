...
export default function Booking({ bookedRooms }) {
    ...
    <div>
      ...
      {bookedRooms.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {bookedRooms.map((roomDetails, index) => (
            <div
              key={index}
              className="border border-gray-200 rounded-lg overflow-hidden hover:shadow-md transition"
            >
              <div className="bg-gray-100 px-4 py-3 border-b border-gray-200">
                <h3 className="font-medium">חדר מספר {roomDetails.room.roomNumber}</h3>
                <h3 className="font-medium"> מספר הזמנה {roomDetails.bookingNumber}</h3>
              </div>
              <div className="p-4 space-y-4">
                <div className="w-full h-full"><Image src={roomDetails.room.imageUrl} width={300} height={200} alt="תמונת החדר"/></div>
                  ...
                  <div>
                    ...
                    <p className="font-medium">
                      ...
                      <span className="text-sm text-gray-500">
                        ({formatRelativeDate(roomDetails.createdAt)} - {formatDistanceFromNow(roomDetails.createdAt)})
                      </span>
                    </p>
                  </div>
                </div>
